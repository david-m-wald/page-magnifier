(() => {
  ...
  const magRegionCX = magRegion.offsetLeft + magRegion.offsetWidth / 2;
  const magRegionCY = magRegion.offsetTop + magRegion.offsetHeight / 2;
  ...
  const wheelMagStep = 0.25;          //Magnification increment for mouse wheel scroll
  const wheelMagLLimit = 1;           //Lower magnification limit
  const wheelMagULimit = 4;           //Upper magnification limit
  ...
  window.positionMagnifier = function(event) {
  ...
  window.positionMagnifier({pageX: magRegionCX, pageY: magRegionCY});
})();
